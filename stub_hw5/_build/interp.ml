...
and
  ...
  | Case(cond,branches) ->
    (match (eval_expr en cond) with
     |TaggedVariantVal(tag, args) ->
        let matcher = function Branch(t, ids, _) -> (tag = t) && (List.length args = List.length ids) in
        (match List.find_opt matcher branches with
          | None -> failwith "Error: No match found"
          | Some (Branch(_,ids,e)) ->
            let et = List.fold_left2 extend_env en ids args in eval_expr et e)
     | _ -> failwith "Error: No TaggedVariantVal Found.")
  | Debug ->
